...
{/* Badge + date */}
        <div className="flex flex-wrap items-center gap-3">
          <span className="rounded-full bg-deepest px-3 py-1 text-xs font-medium text-ink/60">
            {event.type}
          </span>
        ...
        <div className="mt-2 flex flex-nowrap items-center justify-between gap-4">
          ...
          {status === 'completed' ? (
            <span className="shrink-0 rounded-full border border-deepest px-5 py-2 text-sm font-medium text-ink/50">
              Completed
            </span>
          ) : (
            <div className="flex shrink-0 items-center gap-3">
              {status === 'ongoing' && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-accent px-3 py-1 text-xs font-semibold text-on-accent">
        ...
        <div className="mt-3 flex flex-wrap gap-5 text-sm text-ink/60">
          {event.timeframe && (
            <span className="flex items-center gap-2">
              <svg className="h-4 w-4 shrink-0 text-ink/40" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        {event.schedule && event.schedule.length > 0 && (
          <div className="mt-8">
            <h3 className="mb-4 text-sm font-semibold uppercase tracking-widest text-ink/50">
              Program Schedule
            </h3>
            <div className="overflow-hidden rounded-xl border border-deepest">
              ...
              <div className="grid grid-cols-[7rem_1fr] border-b border-deepest bg-deepest sm:grid-cols-[10rem_1fr]">
                <div className="px-4 py-2.5 text-xs font-semibold uppercase tracking-widest text-ink/60">
                  Time
                </div>
                <div className="border-l border-background-dark px-4 py-2.5 text-xs font-semibold uppercase tracking-widest text-ink/60">
                  Activity
                </div>
              </div>
              {/* Rows */}
              {event.schedule.map((item, i) => (
                ...
                >
                  <div className="px-4 py-3 text-xs font-medium leading-snug text-ink/50">
                    {item.time}
                  </div>
